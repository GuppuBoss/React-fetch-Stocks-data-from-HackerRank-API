...
test('initial UI is rendered as expected', () => {
	...
	expect(getByTestId('app-title')).toHaveTextContent('Stock Data');
...
test('search is made on by clicking on search button and result found - test 1', async() => {
  ...
  fetchMock.getOnce(url, JSON.stringify({ page:1,per_page:10,total:0,total_pages:0,data:[{"date":"5-January-2000","open":5265.09,"high":5464.35,"low":5184.48,"close":5357}]}));
  fireEvent.input(input, {
...
test('search is made on by clicking on search button and result found - test 2', async() => {
  ...
  fetchMock.getOnce(url, JSON.stringify({ page:1,per_page:10,total:0,total_pages:0,data:[{"date":"5-January-2001","open":4116.34,"high":4195.01,"low":4115.35,"close":4183.73}]}));
  fireEvent.input(input, {
